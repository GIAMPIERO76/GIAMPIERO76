...
/***************************************************************************
*
*	ShippingTheme - copyright (c) - designercreativo.it
*	The only Italian platform for your freight transport.
*
*	Coder: Cristiano Ottonello
*	Email: info[at]designercreativo.it | info[at]designercreativo.it
*	More info about the theme here: http://designercreativo.it/
*	since v1.0.0.0
*
***************************************************************************/
...
if(isset($_POST['project_submit1']))
{
		...
		{ 
			$projectOK 							= 0;
			$MYerror['project_title'] 					= __('You cannot leave the request title blank!','ProjectTheme'); 
		...
		{ 
			$projectOK = 0; 
			$MYerror['project_category'] 	= __('You cannot leave the request category blank!','ProjectTheme');
		...
		if($ProjectTheme_enable_project_location != "no"):
			...
			{ 
				$projectOK = 0; 
				$MYerror['project_location'] 	= __('You cannot leave the request location blank!','ProjectTheme');
		...
		{ 
			$projectOK = 0; 
			$MYerror['project_description'] 	= __('You cannot leave the request description blank!','ProjectTheme');
		...
		// set request details, as meta post
			  
		update_post_meta($pid, "Location", 		$project_location_addr); // set request location
		update_post_meta($pid, "price", 		ProjectTheme_get_budget_name_string_fromID($_POST['budgets'])); // set request price
...
//************************ STEP 2 SUBMIT *********************************
	...
	{	
		...
		//-----------------------------------
		// see if the request was featured
		...
		//-----------------------------------
		// mark the request for private bids if selected
		
		if(isset($_POST['private_bids'])) update_post_meta($pid, "private_bids", "1");
		else update_post_meta($pid, "private_bids", "0");
		
		
		// mark the request hidden from search engines or people not logged in
